...
# sample haplotypes in NEXT GENERATION and record counts   
	...
	p1 = hp[i,1]+hp[i,2]		# allele prop at loc 1  (same code as above)
	p2 = hp[i,1]+hp[i,3]		# allele prop at loc 2
	
# compute D'_00
	D00 = hp[i,1]-p1*p2
	if(D00>0) ldstat[i,1] = D00/min(p1*(1-p2),p2*(1-p1)) # if greater than
        if(D00<=0) ldstat[i,1] = -D00/min(p1*p2,(1-p2)*(1-p1)) # if less than/ equal to zero 
...
# compute r^2_00
 ...
 # LOOP   
  ...
  return(c(hp[ngen,],ldstat[ngen,])) #hp and ldstat defined at the beginning; row ngen used to get output
