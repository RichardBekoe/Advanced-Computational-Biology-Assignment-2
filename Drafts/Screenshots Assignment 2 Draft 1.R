...
# Code repeated for each population
# adds every other 2nd row and puts the result into a matrix i.e. heterozygosity
# = 1 the number of heterozygous / total number of participants (2 rows per
...
# first two individuals first five snps
...
# Which.max(pA) returns the max allele A frequency
# Which.max(hp[,1:1]) returns max haplotype AB
# Creating a data.frame for the maxA values
...
for (i in 1:npop) {
  ...
  # "AB","Ab","aB","ab","A","B","maxA" If Output[1] (AB) equals to 1
  # (i.e.fixation) then add the Output[7] (the max(pA) value to the data.frame
  # called maxA_values.df)
}

# Creation of a loop to simulate every iteration of a population (total sized npop); 
# as you can only simulate one population at a time using the function

median(maxA_values.df$maxA_values,na.rm=TRUE)
# find the median of the values in maxA_values.df hence the median fixation time

# b) ----------------------------------------------------------------------

maxA_values.df <- data.frame(maxA_values = NA)
# adding some recombination; hence changing the parameters
npop=1000
for (i in 1:npop) { 
...
median(maxA_values.df$maxA_values,na.rm=TRUE)
# to run with new y output from AB instead of from Allele A frequency

# c) ----------------------------------------------------------------------

# setting values so that the selected allele A has initial frequency 0.01
# And varying s; using haplotype AB output 
...
# Increase of rho and repeat exercise c)
maxA_values.df <- data.frame(maxA_values = NA)
npop=1000
for (i in 1:npop) { 
  Output <- ldsel(ngen=5000,nall=2000,init=c(0.005,0.005,0.495,0.495),rho=0.3,s=0.02,mu=0)
  if (Output[1] == 1) { maxA_values.df <- rbind(maxA_values.df, Output[7])
  }
}
median(maxA_values.df$maxA_values,na.rm=TRUE)
